...
2. 醫療知識庫 (Medical AI)
   設備知識檢索 (get_device_knowledge)：針對 Microlife 醫療器材（血壓計、血氧儀等）的官方說明書、錯誤代碼 (Err) 與故障排除流程進行精準匹配。

健康數據趨勢：自動分析用戶歷史生理數據（血壓、心率），識別異常趨勢並標記臨床風險（[EMERGENCY] / [NORMAL]）。

動態視覺化：根據分析結果與用戶需求，自動繪製專業的健康趨勢圖表（折線圖、長條圖、散佈圖）。

(另外研究 Medical RAG RAG 知識檢索 search_device_manual)
...
Windows：連點兩下 run_tests.bat。

指令行：輸入 pytest tests/unit/test_nodes.py。

檢查重點：

test_router_logic：確保機器人不會帶錯路。

test_query_parser：確保日期抓得準。

test_health_analyst_emergency：確保危急時有彈出 [EMERGENCY] 警告。
